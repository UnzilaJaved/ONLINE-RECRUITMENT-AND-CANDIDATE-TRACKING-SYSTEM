...
// Returns everything AdminDashboard.jsx needs in one call:
//   counts, pipeline (per-status), recentApplications (8), upcomingInterviews (5)
router.get('/dashboard', async (req, res) => {
  try {
    ...
    // experience is now on applications table (not candidates)
    const { data: recentApps, error: recentError } = await supabase
      ...
      .select(`
        ...
        experience,
        jobs(title),
        candidates(user_id, profiles(full_name))
      `)
    ...
    // 4. Upcoming interviews (scheduled, from now, next 5)
